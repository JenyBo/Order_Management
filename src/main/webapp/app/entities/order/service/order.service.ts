...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'importDate' | 'exportDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      importDate: order.importDate?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      importDate: restOrder.importDate ? dayjs(restOrder.importDate) : undefined,
  ...
  importOrdersFromExcel(file: File): Observable<HttpResponse<any>> {
    const formData: FormData = new FormData();
    formData.append('file', file, file.name);
    return this.http.post<any>(`${this.resourceUrl}/import`, formData, { observe: 'response' });
  }

  exportOrdersToExcel(): Observable<any> {
    return this.http.get<any>(`${this.resourceUrl}/export-orders`, { responseType: 'blob' as 'json' });
  }
}
